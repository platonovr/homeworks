...
public class Server {
	public static void main(String[] ar) {
		int port = 6662; // случайный порт (может быть любое число от 1025 до
							// 65535)
		try {
			ServerSocket ss = new ServerSocket(port); // создаем сокет сервера и
														// привязываем его к
			...
			while (true) {
				...
				if (line.equals("Hello")) {
					...
					out.writeUTF(line.substring(1, 3));
					out.flush();
				} else {
					System.out
					...
					out.writeUTF(line); // отсылаем клиенту обратно ту самую
										// строку
										// текста.
